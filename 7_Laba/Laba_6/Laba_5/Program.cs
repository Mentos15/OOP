...
namespace Laba_5
{
    ...
    class Book:Uchebnik
    {
        ...
        public Book(int colstr,string name,string janr, int year)
        {
            ...
            if (year > 2019 || year < 0)
            {
                throw new Iskl1("Не правильно указан год выпуска книги");
            }
            else
            {
                Year = year;
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                //Book boook6 = new Book(239, "ОНО", "Ужасы", 2010);
                //ControllerBiblioteki.index();
                //ControllerBiblioteki.delnull();
                //bibl.Del(jurn);
                //string str50 = null;
                //str50.ToString();

                //Biblioteka bibl2 = new Biblioteka();
                //ControllerBiblioteki.masss(null);
                //ControllerBiblioteki.METHOD();
                int[] aa = null;
            ...
            catch(NullReferenceException kkk)
            {
                WriteLine($"Ошибка: NullReferenceException {kkk.TargetSite}");
            }
                 
            catch(Iskl2 isk)
            {
                WriteLine($"{isk.Message}");
            }
            catch(IndexOutOfRangeException )
            {
                WriteLine($"Выход за пределы массива");
            }
            catch(Iskl3 )
            {
